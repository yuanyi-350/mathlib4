...
lemma plus1 (m : ℤ) : 2 ∣ m * (m + 1) := by
  ...
  · have : Even (m * (m + 1)) := Int.even_mul_succ_self m
...
lemma Int.sq_mod_eight_eq_one_of_odd {k : ℤ} (hk : Odd k) : 8 ∣ k ^ 2 - 1 := by
  rcases hk with ⟨m, hm⟩
  have eq : (2 * m + 1) ^ 2 - 1 = 4 * m * (m + 1) := by ring
  have : 4 * 2 ∣ 4 * (m * (m + 1)) := (Int.mul_dvd_mul_iff_left (by norm_num)).mpr (plus1 m)
